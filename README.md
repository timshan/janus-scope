...
> **目前狀態：專案基線建立中，尚未提供可供一般使用者操作的正式版本。**
...
MVP 完成後，希望一般使用者只需要接觸下列操作：

1. 第一次使用：執行 `setup.bat` 準備可攜式 runtime。
2. 平常使用：執行 `start.bat`。
3. 選擇 Desktop 或 Android Mobile 模式後開啟網站。
4. 正常瀏覽網頁，需要保存畫面時使用 JanusScope 的截圖功能。
5. 需要更新瀏覽器元件時執行 `update.bat`。

上述腳本目前仍屬規劃中的 MVP 介面；實際完成狀態以 Releases、Issues 與 `docs/USAGE.md` 為準。
